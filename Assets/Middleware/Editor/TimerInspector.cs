...
{
    [CustomEditor(typeof(UnityTimer))]
    public class TimerInspector : UnityEditor.Editor
    {
        FieldInfo activeTaskClsInfo;
        List<UnityTimer.TimerTask> timerTasks = null;

        private void OnEnable()
        {
            activeTaskClsInfo =
                typeof(UnityTimer).GetField("s_ActiveTasks", BindingFlags.Static | BindingFlags.NonPublic);
        }

        public void RefreshActiveTimerTasks()
        {
            object v = activeTaskClsInfo.GetValue(null);
            timerTasks = v as List<UnityTimer.TimerTask>;
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (timerTasks != null)
            {
                for (int i = 0; i < timerTasks.Count; i++)
                {
                    ...
                    if (null != task.Func)
                    {
                        string caller = task.Func.Target == null
                            ? task.Func.Method.DeclaringType.FullName
                            : task.Func.Target.GetType().FullName.Split('+')[0];
                    ...
                    else
                    {
                        ...
                        if (EditorGUILayout.LinkButton("Jump"))
                        {
                            EditorUtility.DisplayDialog("Error", "The invoking mode of the timer is incorrect!",
                                "Confirm");
                        }
            ...
            if (clssAssetGuids.Length > 0)
            {
                ...
                var scripts = Array.FindAll<string>(clssAssetGuids, guid =>
                {
                    ...
                    if (path.EndsWith(".cs"))
                    {
                        ...
                        string[] classs = Array.FindAll<string>(File.ReadAllLines(path),
                            l => { return l.Contains(classFlag); });
                ...
                Object obj = AssetDatabase.LoadAssetAtPath<UnityEngine.Object>(
                    AssetDatabase.GUIDToAssetPath(string.IsNullOrEmpty(script) ? clssAssetGuids[0] : script));
